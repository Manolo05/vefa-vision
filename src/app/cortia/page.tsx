...
interface Dossier {
  ...
  analyse: Analyse | null; synthese: string | null;
...
function analyser(emp: Emprunteur, proj: Projet, charges: Charge[] = []): Analyse {
  ...
  const taux = 0.035;
  const dur = proj.duree || 240;
  const tm = taux / 12;
  const mens = tm > 0
    ? besoin * (tm * Math.pow(1 + tm, dur)) / (Math.pow(1 + tm, dur) - 1)
    : besoin / dur;
  ...
  if (emp.contrat === 'cdi' || emp.contrat === 'fonctionnaire')
    forces.push(CONTRATS[emp.contrat] + ' — stabilite professionnelle');
  if ((emp.anciennete || 0) >= 24)
    forces.push(Math.floor(emp.anciennete / 12) + " ans d'anciennete");
  ...
  if (te > 35) {
    faiblesses.push('Endettement ' + te + '% > seuil HCSF 35%');
    recos.push("Allonger la duree ou augmenter l'apport");
  }
  if (ratioApp < 5) {
    faiblesses.push('Apport faible (' + ratioApp + '%)');
    recos.push("Augmenter l'apport pour couvrir les frais");
  }
  if (ravUc < 500) faiblesses.push('Reste a vivre insuffisant (' + ravUc + ' EUR/UC)');
  if (emp.contrat === 'cdd' || emp.contrat === 'interim')
    faiblesses.push('Contrat ' + CONTRATS[emp.contrat] + ' — precaire');
  if (saut > 300) recos.push("Saut de charge important — preparer l'argumentaire");

  return {
    rev: Math.round(rev), cout, besoin: Math.round(besoin),
    mens: Math.round(mens), te, rav, ravUc, ratioApp,
    saut: Math.round(saut), score, forces, faiblesses, recos, notaire,
  };
}
...
const Badge = ({ s }: { s: string }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 500, color: x.c, background: x.bg, padding: '2px 9px 2px 7px', borderRadius: 16 }}>
      <span style={{ width: 5, height: 5, borderRadius: '50%', background: x.d }} />
      {x.l}
    </span>
  );
};
...
const Ring = ({ v, sz = 40 }: { v: number | null; sz?: number }) => {
  if (v == null) return (
    <div style={{ width: sz, height: sz, borderRadius: '50%', border: '1.5px dashed ' + C.ink08, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: C.ink15 }}>--</div>
  );
  const r = (sz - 6) / 2;
  ...
  return (
    <div style={{ position: 'relative', width: sz, height: sz, flexShrink: 0 }}>
      <svg width={sz} height={sz} style={{ position: 'absolute' }}>
        <circle cx={sz / 2} cy={sz / 2} r={r} fill="none" stroke={bg} strokeWidth={3} />
        <circle cx={sz / 2} cy={sz / 2} r={r} fill="none" stroke={col} strokeWidth={3}
          strokeDasharray={circ} strokeDashoffset={offset} strokeLinecap="round"
          style={{ transform: 'rotate(-90deg)', transformOrigin: 'center', transition: 'stroke-dashoffset 0.8s cubic-bezier(.4,0,.2,1)' }} />
      </svg>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: sz > 50 ? 16 : 12, fontWeight: 700, color: tc }}>{v}</div>
    </div>
  );
};
  ...
  <div style={{ marginBottom: 12 }}>
    ...
    {options ? (
      <select value={String(value)} onChange={e => onChange(e.target.value)}
        style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid ' + C.ink08, fontSize: 13, background: C.white }}>
      ...
      <input type={type} value={value}
        ...
        style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid ' + C.ink08, fontSize: 13, boxSizing: 'border-box' }} />
    )}
  </div>
);

const Btn = ({ children, onClick, primary, disabled, full }: {
  children: React.ReactNode; onClick?: () => void;
  primary?: boolean; disabled?: boolean; full?: boolean;
}) => (
  <button onClick={onClick} disabled={disabled}
    style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px 18px', borderRadius: 8, border: primary ? 'none' : '1px solid ' + C.ink08, background: primary ? C.ink : C.white, color: primary ? C.white : C.ink70, fontSize: 13, fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.5 : 1, width: full ? '100%' : 'auto', transition: 'all 0.15s' }}>
    {children}
  </button>
);
...
export default function CortIA() {
  ...
  useEffect(() => {
    const d = loadData<Dossier[]>('cortia-dossiers', []);
    setDossiers(d);
    setLoaded(true);
  }, []);

  useEffect(() => {
    if (loaded) saveData('cortia-dossiers', dossiers);
  }, [dossiers, loaded]);
  ...
  function createDossier(data: { titre: string; emp: Emprunteur; proj: Projet }) {
    ...
    const d: Dossier = {
      id, ref, statut: 'en_collecte', score: null,
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
      analyse: null, synthese: null, charges: [], ...data,
    };
  ...
  function runAnalyse() {
    ...
    updateDossier(current.id, {
      analyse: r, score: r.score,
      statut: r.te > 35 ? 'a_corriger' : 'en_analyse',
    });
  }
  ...
  async function genererSynthese() {
    ...
    setAiLoading(true);
    try {
      const response = await fetch('/api/cortia-synthese', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ analyse: current.analyse, emp: current.emp, proj: current.proj }),
      });
      const data = await response.json();
      if (data.error) throw new Error(data.error);
      const note: string = data.note || 'Erreur de generation';
      updateDossier(current.id, { synthese: note, statut: 'pret_banque' });
      setAiResult(note);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Erreur inconnue';
      setAiResult('Erreur : ' + msg);
    }
    setAiLoading(false);
  }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'DM Sans', -apple-system, system-ui, sans-serif", background: C.ink02 }}>
      <aside style={{ width: 220, background: C.ink, display: 'flex', flexDirection: 'column', flexShrink: 0, position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <nav style={{ flex: 1, padding: '10px 8px' }}>
          {nav.map(n => (
            <button key={n.id} onClick={() => { setPage(n.id); if (n.id === 'nouveau') { setCurrent(null); setAiResult(null); } }}
      ...
      <main style={{ flex: 1, padding: '24px 28px', overflowY: 'auto', maxHeight: '100vh' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: C.ink30, margin: '3px 0 0' }}>{dossiers.length} dossier{dossiers.length !== 1 ? 's' : ''} au total</p>
              </div>
              ...
              <div style={{ background: C.white, borderRadius: 12, border: '1px solid ' + C.ink08, overflow: 'hidden' }}>
                ...
                {dossiers.slice(0, 10).map(d => (
                  <div key={d.id} onClick={() => openDossier(d)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 18px', borderBottom: '1px solid ' + C.ink04, cursor: 'pointer' }}>
                    <Ring v={d.score} sz={36} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: C.ink }}>{d.titre || 'Sans titre'}</div>
                      <div style={{ fontSize: 11, color: C.ink30 }}>{d.ref} — {d.emp?.prenom} {d.emp?.nom}</div>
                    </div>
                    <Badge s={d.statut} />
        ...
        {page === 'dossiers' && (
          <div>
            ...
            <div style={{ background: C.white, borderRadius: 12, border: '1px solid ' + C.ink08, overflow: 'hidden' }}>
              {dossiers.length === 0 ? (
                <div style={{ padding: 48, textAlign: 'center', color: C.ink30 }}>Aucun dossier</div>
              ) : dossiers.map(d => (
                <div key={d.id} onClick={() => openDossier(d)}
                  style={{ display: 'grid', gridTemplateColumns: '40px 1fr 100px 80px 60px', gap: 10, alignItems: 'center', padding: '11px 18px', borderBottom: '1px solid ' + C.ink04, cursor: 'pointer' }}>
                  <Ring v={d.score} sz={34} />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{d.titre}</div>
                    <div style={{ fontSize: 11, color: C.ink30 }}>{d.ref}</div>
                  </div>
                  <Badge s={d.statut} />
        ...
        {page === 'dossier' && current && (
          <DossierPage
            d={current} tab={tab} setTab={setTab}
            onUpdate={(u) => updateDossier(current.id, u)}
            onAnalyse={runAnalyse}
            onSynthese={genererSynthese}
            aiLoading={aiLoading}
            aiResult={aiResult || current.synthese || null}
          />
        )}
      </main>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        {[1, 2, 3].map(s => <div key={s} style={{ flex: 1, height: 4, borderRadius: 2, background: s <= step ? C.ink : C.ink08, transition: 'background 0.3s' }} />)}
      </div>
        ...
        {step === 1 && (
          <>
            ...
            <div style={{ marginTop: 16 }}>
              <Btn primary onClick={() => { if (titre) { setProj(p => ({ ...p, type_projet: tp })); setStep(2); } }} disabled={!titre}>Suivant</Btn>
            </div>
          </>
        )}
        {step === 2 && (
          <>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              ...
              <Btn primary onClick={() => { if (emp.prenom && emp.nom) setStep(3); }}>Suivant</Btn>
            </div>
          </>
        )}
        {step === 3 && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <Input label="Duree" value={proj.duree} onChange={v => setProj(p => ({ ...p, duree: Number(v) }))}
                options={[['120', '10 ans'], ['180', '15 ans'], ['240', '20 ans'], ['300', '25 ans']]} />
            </div>
            {proj.prix > 0 && (
              <div style={{ background: C.ink02, borderRadius: 8, padding: 12, marginTop: 12, fontSize: 12, color: C.ink50 }}>
                Frais notaire est. : {Math.round(proj.prix * 0.078).toLocaleString('fr-FR')} EUR
                {' — '}Besoin : {Math.round(proj.prix * 1.078 - proj.apport).toLocaleString('fr-FR')} EUR
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              <Btn onClick={() => setStep(2)}>Retour</Btn>
              <Btn primary onClick={() => onCreate({ titre, emp, proj })}>Creer le dossier</Btn>
            </div>
          </>
        )}
      </div>
...
// FICHE DOSSIER
// ============================================
function DossierPage({ d, tab, setTab, onUpdate, onAnalyse, onSynthese, aiLoading, aiResult }: {
  d: Dossier; tab: string; setTab: (t: string) => void;
  onUpdate: (u: Partial<Dossier>) => void;
  onAnalyse: () => void; onSynthese: () => void;
  aiLoading: boolean; aiResult: string | null;
}) {
  const tabs = ['emprunteur', 'projet', 'analyse', 'synthese'];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 2, marginBottom: 20, borderBottom: '1px solid ' + C.ink08 }}>
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{ padding: '8px 16px', fontSize: 13, fontWeight: tab === t ? 600 : 400, color: tab === t ? C.ink : C.ink30, borderBottom: tab === t ? '2px solid ' + C.ink : '2px solid transparent', background: 'none', border: 'none', cursor: 'pointer', marginBottom: -1, textTransform: 'capitalize' }}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div style={{ maxWidth: 640 }}>
        ...
        {tab === 'projet' && (
          <div style={{ background: C.white, borderRadius: 12, border: '1px solid ' + C.ink08, padding: 20 }}>
            ...
            <Input label="Duree" value={proj.duree || 240} onChange={v => setProj(p => ({ ...p, duree: Number(v) }))}
              options={[['120', '10 ans'], ['180', '15 ans'], ['240', '20 ans'], ['300', '25 ans']]} />
            ...
            {!a ? (
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>Score : {a.score}/100</div>
                    <div style={{ fontSize: 12, color: C.ink30 }}>Financabilite</div>
                  </div>
                ...
                {a.forces.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: C.emD, marginBottom: 6 }}>Forces</div>
                    {a.forces.map((f, i) => <div key={i} style={{ fontSize: 12, background: C.emL, color: C.emD, padding: '6px 10px', borderRadius: 6, marginBottom: 4 }}>{f}</div>)}
                  </div>
                )}
                {a.faiblesses.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: C.rdD, marginBottom: 6 }}>Faiblesses</div>
                    {a.faiblesses.map((f, i) => <div key={i} style={{ fontSize: 12, background: C.rdL, color: C.rdD, padding: '6px 10px', borderRadius: 6, marginBottom: 4 }}>{f}</div>)}
                  </div>
                )}
                {a.recos.length > 0 && (
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: C.brand, marginBottom: 6 }}>Recommandations</div>
                    {a.recos.map((r, i) => <div key={i} style={{ fontSize: 12, background: C.brandL, color: C.brand, padding: '6px 10px', borderRadius: 6, marginBottom: 4 }}>{r}</div>)}
                  </div>
                )}
              </div>
            )}
        ...
        {tab === 'synthese' && (
          <div>
            {!a ? (
              <div style={{ background: C.white, borderRadius: 12, border: '1px solid ' + C.ink08, padding: 40, textAlign: 'center' }}>
                <p style={{ color: C.ink30, marginBottom: 12 }}>Lancez d'abord une analyse</p>
              </div>
            ) : (
              <div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                  <Btn primary onClick={onSynthese} disabled={aiLoading}>
                    {aiLoading ? 'Generation en cours...' : aiResult ? 'Regenerer' : 'Generer la note bancaire'}
                  </Btn>
                  {aiResult && <Btn onClick={() => { navigator.clipboard.writeText(aiResult || ''); }}>Copier</Btn>}
                </div>
                {aiResult && (
                  <div style={{ background: C.white, borderRadius: 12, border: '1px solid ' + C.ink08, borderLeft: '3px solid ' + C.brand, padding: 20 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: C.ink50, marginBottom: 10 }}>Note bancaire premium</div>
                    <div style={{ fontSize: 13, color: C.ink70, lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{aiResult}</div>
                  </div>
                )}
              </div>
            )}
