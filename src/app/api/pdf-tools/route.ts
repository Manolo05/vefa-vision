...
function pdfResponse(bytes: Uint8Array, filename: string, extraHeaders?: Record<string, string>) {
  ...
  try {
    ...
    if (action === "merge") {
      if (files.length < 2)
        return NextResponse.json(
          { error: "Au moins 2 fichiers requis" },
          { status: 400 }
        );
      ...
      return pdfResponse(mergedBytes, `merged_${Date.now()}.pdf`);
    }

    if (action === "split") {
      if (files.length !== 1)
        return NextResponse.json(
          { error: "Un seul fichier requis" },
          { status: 400 }
        );
      ...
      if (!rangeStr)
        return NextResponse.json({
          ...
          info: true,
          totalPages,
          message: `PDF de ${totalPages} pages. Specifiez une plage (ex: "1-3,5").`,
        });
      ...
      return pdfResponse(newBytes, `split_${Date.now()}.pdf`);
    }
    ...
    if (action === "compress") {
      ...
      const ratio = Math.round(
        (1 - compressedBytes.length / buffer.length) * 100
      );
      return pdfResponse(compressedBytes, `compressed_${Date.now()}.pdf`, {
        "X-Original-Size": buffer.length.toString(),
        "X-Compressed-Size": compressedBytes.length.toString(),
    ...
    if (action === "rotate") {
      ...
      return pdfResponse(rotatedBytes, `rotated_${Date.now()}.pdf`);
    }
    ...
    if (action === "watermark") {
      ...
      const opacity = parseFloat(
        (formData.get("opacity") as string) || "0.15"
      );
      const buffer = Buffer.from(await files[0].arrayBuffer());
      const pdfDoc = await PDFDocument.load(buffer, {
        ignoreEncryption: true,
      ...
      return pdfResponse(watermarkedBytes, `watermarked_${Date.now()}.pdf`);
    }
    ...
    if (action === "annotate") {
      ...
      const pageNum =
        parseInt((formData.get("page") as string) || "1") - 1;
      const color = (formData.get("color") as string) || "yellow";
      if (!annotationText)
        return NextResponse.json(
          { error: "Texte d'annotation requis" },
          { status: 400 }
        );
      ...
      return pdfResponse(annotatedBytes, `annotated_${Date.now()}.pdf`);
    }
    ...
    if (action === "sign") {
      ...
      const widthPct = parseFloat(
        (formData.get("widthPct") as string) || "20"
      );
      if (!signatureBase64)
        return NextResponse.json(
          { error: "Image de signature requise" },
          { status: 400 }
        );
      ...
      const sigBuffer = Buffer.from(
        signatureBase64.replace(/^data:image/[a-z]+;base64,/, ""),
        "base64"
      );
      ...
      return pdfResponse(signedBytes, `signed_${Date.now()}.pdf`);
    }
    ...
    if (error?.message?.includes("encrypted"))
      return NextResponse.json(
        { error: "PDF protege par mot de passe." },
        { status: 422 }
      );
    return NextResponse.json(
